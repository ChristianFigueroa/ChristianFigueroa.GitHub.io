...
window.fontTeX = (function() {
  ...
  // Anything else will return a 0-long Array.
  ...
  function parsedFontTeXInstanceList(elements) {
    if (typeof elements == "string") {
      ...
      let filtered = [];
      let elementList = texElements.map(function(instance) {
      ...
      return texElements.map(function(element) {
        return element.parsedTeXInstance;
      });
    }
  }
  ...
  fontTeX.rerender = function rerender(elements) {
    ...
    instances = texElements.filter(function(element) {
      return instances.includes(element.parsedTeXInstance);
    });
  ...
  fontTeX.parse = function parse(texString) {
    ...
    function eat(index) {
      ...
      let catcode = data.cats[char.charCodeAt(0)];
      if (!catcode) {
        return [{}, 0];
      }
    ...
    while (texString) {
      ...
      if (texString.charCodeAt(0) in data.cats) {
        ...
        if (type == "display" || type == "inline") {
          ...
          // `tokenize` returns a 2-long array. The first item is an array of tokens from parsing.
          // The second item is the string that was left over after parsing.
          if (!tokens[2]) {
  ...
  fontTeX.ParsedFontTeX.prototype = {
    renderIn: function renderIn(elements) {
      ...
      let tokens = this.parsedTokens.slice();
      for (let i = list.length - 1; i >= 0; i--) {
        ...
        if (list[i].hasAttribute('data-fontTeX-container-id')) {
          ...
          texElements[index].parsedTeXInstance = this;
        } else {
        ...
        list[i].setAttribute("data-fontTeX-input", this.texString);
  ...
  // `tokenize' function defined below. It converts a string of TeX into an array
  ...
  function tokenize(texString, style) {
    ...
    // Make a new Scope for the array that will act as the outermost scope. Each Scope automatically
    // gets added to `scopes` in its constructor.
    new Scope();
  ...
  fontTeX.format = function global(texString) {
    var scope = tokenize(texString + '$', 'inline', true);
  ...
  function genHTML(container, tokens, contStyle, cssDeclaration) {
    ...
    function newBox(tokens, style, cramped, font, parent) {
      ...
      childFlexes.last = function() {
        return childFlexes[childFlexes.length - 1];
      }
      ...
      // Iterate over the tokens and turn them into HTML elements. We still need to combine those
      // elements inside their parents to have the right spacing between them.
      for (let i = 0, l = tokens.length; i < l; i++) {
        ...
        //   continue;
        // }
        // if (token.type == "atom" && token.atomType == atomTypes.BIN) {
        //   continue;
        // }
        // if (token.type == "atom" && atoms.length && (
        //     token.atomType == atomTypes.REL ||
        //     token.atomType == atomType.CLOSE ||
        ...
        //   continue;
        // }
        if (token.type == "atom" && token.atomType == atomTypes.OP) {
          if (token.limits == "display") {
        ...
        if (token.type == "fraction") {
          ...
          let numerScaledHeight = numerHeight * (textOrScript ? 0.707106781 : 1);
          let numerScaledWidth = numerWidth * (textOrScript ? 0.707106781 : 1);

          let denomHeight = denom.offsetHeight / fontSize;
          let denomWidth = denom.offsetWidth + 1;
          let denomScaledHeight = denomHeight * (textOrScript ? 0.707106781 : 1);
          let denomScaledWidth = denomWidth * (textOrScript ? 0.707106781 : 1);
          ...
          if (textOrScript) {
            numer.style.fontSize = denom.style.fontSize = "0.707106781em";
            ...
            denom.style.top = `calc(${
              denomHeight - axisHeight / .707106781
          ...
          let startGrower = document.createElement("div");
          let endGrower = document.createElement("div");
          ...
          startGrower.style.flexGrow =
              endGrower.style.flexGrow =
              startGrower.style.flexShrink =
              endGrower.style.flexShrink = 1;
            ...
            function renderElem(elem, delimiter, leftSide, scale) {
              ...
              switch (delimiter) {
                ...
                case "⟩":
                  ...
                  elem.style.marginRight = `${fontTeX.fontDimen.widthOf(delimiter, family) - fontTeX.fontDimen.visibleWidthOf(delimiter, family) + fontTeX.fontDimen.leftOffsetOf(delimiter, family)}em`;
                  ...
                  if (scale >= 3) {
                    ...
                    if (elem.height > scale) {
                      ...
                      topHalfRow =
                          context.getImageData(
                            0,
                            Math.ceil(elem.height / 2) + Math.floor(scale / 2) - Math.floor(scale / 3) - 1,
                            elem.width,
                            1
                          ).data;
                      bottomHalfRow = bottomHalf.data.slice(0, elem.width * 4);
      ...
      // Each list is iterated over twice. `tokenToHTML` is the set of steps for the first
      // iteration. It's where each atom is turned into HTML. `placeHTML' is responsible
      // for setting those atoms and glues/kerns into their parents.
      function tokenToHTML(step, i, l) {
        var token = tokens[i],
          next = tokens[i + 1] || {},
          previous = tokens[i - 1] || {};
        switch (step) {
          ...
          default:
            ...
            if (token.type != 'glue' && token.type != 'kern' && token.type != 'vglue' && token.type != 'vkern') return tokenToHTML(2, i, l);
          ...
          case 4:
            ...
            if (token.type == 'atom' && token.atomType == 2) {
              ...
              return tokenToHTML(10, i, l);
            } else return tokenToHTML(5, i, l);
            break;
          ...
          case 9:
            ...
            if (token.type == 'table') {
              ...
              container.removeChild(token.div);
            } else return tokenToHTML(10, i, l);
            break;

          case 10:
            return tokenToHTML(11, i, l);
            break;
      ...
      // `placeHTML' places atoms, kerns, and glues into the correct elements so that the
  ...
  // Since FontTeX depends so heavily on font measurements, having the proper font loaded is a must
  // the first time around. When a browser is loading a page, if FontTeX renders a piece before a
  // font is actually loaded into the page, it could cause wrong measurements to happen. In my test-
  // ing, reloading my page uncached (so that fonts are re-downloaded), sometimes caused minor spa-
  // cing issues. In production, I don't think this should be an issue since FontTeX will also take
  // time to download and run, and by that time, the fonts will have most likely been downloaded and
  // good to go already. And also, most websites use similar sets of fonts, so chances are, the font
  // will be cached already from a previous website they visited. On my local computer, FontTeX was
  // just a local file immediately available to run, before the fonts had been downloaded, so that's
  // why I think its just an issue for me as I'm developing it. I'm still looking for a solution to
  // this, but haven't come across a good cross-browser workaround yet.
  fontTeX.fontDimen = {
    ...
    init: function() {
      // Before the first measurement can be made, a dedicated <canvas> element is set up and hidden
      // away from the user. This is where all the measurements will be made.
    ...
    // Boolean indicating if a <canvas> element has been set up yet.
    isInit: false,
