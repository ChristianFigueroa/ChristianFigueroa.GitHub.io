...
// `uuidv4` function from https://github.com/uuidjs/uuid.
!function(t,e){"object"==typeof exports&&"undefined"!=typeof module?module.exports=e():"function"==
typeof define&&define.amd?define(e):(t=t||self).uuidv4=e()}(this,(function(){"use strict";var t=
"undefined"!=typeof crypto&&crypto.getRandomValues&&crypto.getRandomValues.bind(crypto)||"undefined"
!=typeof msCrypto&&"function"==typeof msCrypto.getRandomValues&&msCrypto.getRandomValues.bind(
msCrypto),e=new Uint8Array(16);function n(){if(!t)throw new Error(
"crypto.getRandomValues() not supported. See https://github.com/uuidjs/uuid#getrandomvalues-not-supported"
);return t(e)}for(var o=[],r=0;r<256;++r)o.push((r+256).toString(16).substr(1));return function(t,e,
r){"string"==typeof t&&(e="binary"===t?new Uint8Array(16):null,t=null);var u=(t=t||{}).random||(t
.rng||n)();if(u[6]=15&u[6]|64,u[8]=63&u[8]|128,e){for(var i=r||0,d=0;d<16;++d)e[i+d]=u[d];return e}
return function(t,e){var n=e||0,r=o;return(r[t[n+0]]+r[t[n+1]]+r[t[n+2]]+r[t[n+3]]+"-"+r[t[n+4]]+r[t
[n+5]]+"-"+r[t[n+6]]+r[t[n+7]]+"-"+r[t[n+8]]+r[t[n+9]]+"-"+r[t[n+10]]+r[t[n+11]]+r[t[n+12]]+r[t[n+13
]]+r[t[n+14]]+r[t[n+15]]).toLowerCase()}(u)}}));
...
window.fontTeX = (function() {
  ...
  // Change one individual option by providing the name of the option and the new value to set it
  // to. Returns the updated value (may be different than the provided value).
  // Change multiple options in one call by providing an object with key-value pairs of the options
  // to change (e.g. {"radical.build": false, "operator.growamount": 2}) as the sole argument. Re-
  // turns the entire object of settings, reflecting any updated values.
  ...
  // change these. Each value is a 2-long array with the 0th element being the value for the corres-
  // ponding option and the 1st element being the type.
  // settings["autoupdate"] - Whether each instance of rendered TeX on the page should be automatic-
  //     ally checked and updated for style changes. Default: true
  // settings["parsehtml"] - Whether HTML text should be interpreted as actual HTML tags or just
  //     plain tags. Default: true
  // settings["invalid.color"] - The color that invalid TeX will be highlighted in. Default: "red"
  ...
  //     More specific details and examples are described after the `settings` initialization.
  //     Default: true
  // settings["radical.w"] - One of three settings that affect how a radical symbol is drawn. View
  //     the section with more details after the `settings` initialization for more information.
  ...
  //     \displaystyle mode to make them bigger than their \textstyle mode counterparts. Default:
  //     1.75
   ...
   * large radical symbol that encompasses the entire fractions). TeX fonts typically have spe-   *
   * cial information embedded in them that allow TeX to know how to scale up a radical symbol    *
   * properly. Normal fonts like those used on the web don't. To compensate and still allow some- *
   * what reasonable-looking radical symbols to be scaled up, they are custom drawn on a <canvas> *
  ...
  // A Node will return a 1-long array of just itself.
  // A NodeList will return an array of its Nodes.
  // An Array will return an Array of the Nodes within the original Array.
  ...
  function parsedFontTeXInstanceList(elements) {
    if (typeof elements == "string") {
      ...
      for (let i = 0, l = elements.length; i < l; i++) {
        ...
        } else if (elements[i] instanceof HTMLElement) {
  ...
  // Used to update elements with rendered FontTeX content within them. Whenever the styling changes
  // for an element with rendered TeX, the TeX elements may need to be re-rendered to match the new
  // styles. Three CSS styles specifically are looked at: color, font-size, and font-family. Any
  // changes to en element's "style" or "class" attributes will automatically cause an update to
  // happen, as well as any updates/removals of nodes to the document. In general, the user should
  // not have to worry about updating styles unless they are manipulating stylesheets directly or u-
  // sing other attributes other than "class" to inherit styles from stylesheets. Returns the number
  // of elements that were updated.
  ...
  // Create a mutation observer that will cause updates whenever styles change in the document.
    ...
    while (texString) {
      ...
      if (texString.codePointAt(0) in data.cats) {
        ...
        if (type == "display" || type == "inline") {
          ...
          // indicating if the TeX group was closed.
          if (tokens[2]) {
            texString = tokens[1];
  ...
  // The `render` function takes an element and converts it into a TeX container. It uses the elem-
  // ent's "use" (or "data-use") attribute as the content, or, if the element has neither of those
  // two attributes, the element's own inner HTML. This allows the user to specify the content of an
  // element directly in the HTML markup instead of separately in a script. Using the "use" attrib-
  // ute allows the element to display its regular HTML as a fall back, and only get replaced once
  // the TeX has actually finished rendering.
  fontTeX.render = function render(elements) {
    elements = elementList(elements);
    for (let i = elements.length - 1; i >= 0; i--) {
      let texString = elements[i].getAttribute("data-fontTeX-input");
      if (texString === null) {
        texString = elements[i].getAttribute("use") ||
            elements[i].getAttribute("data-use") ||
            elements[i].innerHTML;
      }
      fontTeX.parse(texString).renderIn(elements[i]);
      elements[i].setAttribute("data-fontTeX-input", texString);
    }
  }

  // <font-tex> elements are automatically and immediately rendered via the `fontTeX.render` func-
  // tion. This allows the user to use FontTeX without any scripting of their own whatsoever. Look
  // at `fontTeX.render` above for details. Adding a <font-tex> element to the document will render
  // its contents right away using a document-wide MutationObserver.
  let autoRenderingElements = document.getElementsByTagName("font-tex");
  function renderAutoRenderingElements() {
    for (let i = autoRenderingElements.length - 1; i >= 0; i--) {
      if (autoRenderingElements[i].getAttribute("data-fontTeX-is-parsed") != "true") {
        fontTeX.render(autoRenderingElements[i]);
        autoRenderingElements[i].setAttribute("data-fontTeX-is-parsed", "true");
      }
    }
  }
  new MutationObserver(renderAutoRenderingElements).observe(document, {
    childList: true,
    subtree: true
  });
  ...
  fontTeX.ParsedFontTeX.prototype = {
    renderIn: function renderIn(elements) {
      ...
      bodyLoaded.then(function() {
        ...
        for (let i = list.length - 1; i >= 0; i--) {
          ...
          let cssDeclaration;
          if (list[i].hasAttribute("data-fontTeX-container-id")) {
            let index = +list[i].getAttribute("data-fontTeX-container-id");
            ...
            list[i].setAttribute("data-fontTeX-container-id", index);
          ...
          // This makes sure that FontTeX will re-render an element once its font has loaded. If Font-
          // TeX renders TeX inside an element before its font has actually loaded, the measurements
          // will be wrong, even though the CSSDeclaration tells us that we are measuring the correct
          // font. If a new font loads that changes the displayed font of an element, its font meas-
          // urements are cleared and it is re-rendered.
          let fontStack = cssDeclaration.fontFamily;
          onFontLoad(fontStack, function() {
            clearFontCache(fontStack);
          ...
          if (!settings.parsehtml[0]) {
            ...
            // up (to prevent something like "<span></span>$\TeX$<span></span>"). We could just parse
            // a group of TeX tokens, stringify the HTML using the .innerHTML property, and then have
            // one complete string such as "<span><span> [TeX markup] </span></span>", then set that
            // into the element by changing its .innerHTML. FontTeX however uses <canvas> elements to
            // render certain characters, such as \left ... \right delimiters. Stringifying a <canvas>
            // destroys any drawings it may have, so when we place that stringified version into an
            // element, <canvas> elements will appear blank. The only option is to save the exact
            // <canvas> elements produced by `genHTML` and place those elements into the containing
            // element without stringifying it. So we have to stringify everything that is NOT a group
            // of TeX tokens, but NOT stringify everything that is, and then combine those two in one
            // parent element.

            // To accomplish this, every group of TeX tokens get associated with one, unique <div>
            // element, which DOES get stringified. The entire string of HTML will look something like
            // "<span><span data-fontTeX-uuid-[UUID]></span></span>". That string then gets placed in-
            // to the containing element via its .innerHTML property. Every <div> element then gets
            // replaced with the HTML from `genHTML`, which happens without stringifying the replace-
            // ment element. The HTML from the original string gets parsed, and the element from
            // `genHTML` appears in the document without destroying any <canvas> data.

            // This system relies directly on the user's own input of HTML, so it is cannot be free of
            // errors unless the user's own HTML is well-formed. An example of malformed HTML that can
            // mess up this system is
            // "<span class='no-closing-quote>$\TeX$</span>". 
            // The string that will be generated from this will be something like 
            // "<span class='no-closing-quote><span data-fontTeX-uuid-[UUID]></span></span>"
            // The entire <span> was absorbed into the <span>'s class attribute, so there is no target
            // for the TeX tokens to replace. This can only happen with malformed user input, and
            // there's no way (that I know of) to protect against it without making an HTML validator.
            ...
            for (let n = 0, j = tokens.length; n < j; n++) {
              if (typeof tokens[n] == "string") {
                ...
                let uuid = uuidv4();
                elementIds.push([uuid, genHTML(list[i], tokens[n][0], tokens[n][1], cssDeclaration)]);
                html += `<span data-fontTeX-uuid-${uuid}></span>`;
              }
            }
            ...
            for (let n = 0, j = elementIds.length; n < j; n++) {
              let elem = div.querySelector(`[data-fontTeX-uuid-${elementIds[n][0]}]`);
              if (!elem) {
          ...
          list[i].setAttribute("data-fontTeX-input", instance.texString);
          list[i].setAttribute("data-fontTeX-is-parsed", "true");
        }
      }.bind(this));
    },
    ...
    function Mouth(customString, customQueue) {
      ...
      this.eat = function (context) {
        ...
        switch (context) {
          ...
          // space tokens can be returned (normally, whitespace tokens are skipped over and the next
          // token is returned).
          case undefined:
          case "pre space":
          default:
            ...
            if (this.queue.length) {
              // If the next token is a space character, don't return it unless the context is "pre
              // space".
            ...
            // Check if the next token is an escape character, indicating the start of a macro name
            // (usually a "\").
            else if (catOf(this.string[0]) == catcodes.ESCAPE) {
              // If there are no more characters, or just an EOL character, then the command name is
              // empty. The macro name is just the empty string.
              ...
              if (this.string[1] == this.string[2] && catOf(this.string[1]) == catcodes.SUPERSCRIPT
                && this.string[4] && "0123456789abcdef".includes(this.string[3]) &&
              ...
              // Check for what type of command name this is: either a one non-letter character, or
            ...
            // Since whitespace tokens are ignored in math mode, they are basically the same as ig-
            // nored characters. Ignored tokens get removed and the next token is parsed and re-
            // turned instead.
          ...
          // context to get the first token. If it's an opening token, all the tokens up to the clo-
          // sing token will be returned. Otherwise, the single token is returned. An example is
          // \accent. \accent takes two arguments. The first is the code point of the character to
          // use as the accent. The second argument though can be anything, including a group of to-
          // kens. That's where this context comes in.
          case "argument":
          ...
          case "integer":
            // The integer context looks for an integer in the next available tokens. There are dif-
            // ferent syntaxes for numbers; all of them are described in detail in the TeXbook (pg.
            // 269).
            mouth = new Mouth(this.string, this.queue);

            // The `intContext` variable keeps track of which tokens are allowed to appear next. For
            // example, a "-" is allowed at the beginning of the number, but not after any digits
            // have already been found.
            subContext = "start";

            // This variable is always either 1 or -1 and keeps track of the sign of the number. For
            // every "-" that is encountered before the actual digits, this variable is multiplied
            // by -1.
            sign = 1;

            // `digits` keep track of the numbers that have already been parsed. More numbers may be
            // added on before it's finished.
            digits = 0;

            // `found` is a boolean indicating if any actual digits were found. For example, --" is
            // the start of a hexadecimal number, but no digits are actually defined. That has to be
            // differentiated with --"0, which is a valid number translating to just 0.
            found = false;
            ...
            while (true) {
              ...
              // If this is a command or active character, expand it, but only if the token expands
              // to a register.
              if (subContext == "start" && (token.type == "command" || token.type == "character" &&
              ...
              else if (subContext == "start" && token.cat == catcodes.OTHER && token.char == '-') {
                // A minus sign was found. Negate `sign` so that the final number will also be nega-
                // ted.
                sign *= -1;
              }
              else if (subContext == "start" && token.cat == catcodes.OTHER && token.char == '+') {
                ...
                // A register was found, like \count1 or \escapechar. It doesn't matter what type of
                // register it is yet because all registers can be coerced into integers.
                if (token.type == "integer") {
                  ...
                  // how many pixels 1em actually translates to without knowing the element's font-
                  // size.
          ...
          case "dimension":
            ...
            while (!foundUnit) {
              ...
              if (subContext == "start" && (token.type == "command" || token.type == "character" &&
                ...
                if (secondLetter && (secondLetter.char == "m" || secondLetter.char == "M") &&
                ...
                else if (secondLetter && (secondLetter.char == "x" || secondLetter.char == "X") &&
                  secondLetter.cat != catcodes.ACTIVE) {
                  ...
                  // don't know the font yet since we don't have access to the elements. 1ex is as-
                  // sumed to be (9107 / 19200) em, which is about 0.474322916667 of an em. That
                ...
                else if (secondLetter && (secondLetter.char == "c" || secondLetter.char == "C") &&
                  secondLetter.cat != catcodes.ACTIVE) {
                  sp = parseFloat((digits + " ").replace(" .", ".").replace(". ", "") || '0') *
                    65536 * 12;
                  foundUnit = true;
          ...
          case "glue":
            ...
            // used here to get the three dimensions, along with the factor context to get the fac-
            // tor of any fil(l)(l)s.
            mouth = new Mouth(this.string, this.queue);
            subContext = "start";
            sign = 1;
            mouthContext = "pre space";
            ...
            while (true) {
              ...
              else if ((subContext == "post start" || subContext == "stretch signs") && foundStretch) {
                mouthContext = "dimension";
                mouth.revert();
              }
              ...
              else if (subContext == "post stretch" && foundShrink && token.cat == catcodes.OTHER
                && token.char == "-") {
                shrinkSign *= -1;
              ...
              else if ((subContext == "post stretch" || subContext == "shrink signs" || subContext
                == "post start") && foundShrink && token.register && token.type == "dimension") {
                shrink = new DimenReg(token.sp.value * shrinkSign, token.em.value * shrinkSign);
                break;
              }
              else if ((subContext == "post stretch" || subContext == "shrink signs" || subContext
                == "post start") && foundShrink && token.register && token.type == "integer") {
                let f = mouth.eat();
                if (f && (f.char == "f" || f.char == "F") && f.cat != catcodes.ACTIVE) {
                  let i = mouth.eat("pre space");
                  if (i && (i.char == "i" || i.char == "I") && i.cat != catcodes.ACTIVE) {
          ...
          case "mu glue":
            ...
            while (true) {
              ...
              if (!token && !((subContext == "post start" || subContext == "stretch signs") &&
                mouthContext == "mu dimension") && !((subContext == "post stretch" || subContext
                  == "shrink signs") && mouthContext == "mu dimension")) {
                if (lastState) {
                  mouth.loadState(lastState);
                }
                break;
              ...
              if (!token && mouthContext == "mu dimension") {
                ...
                == "post start") && foundShrink && token.register && token.type == "mu dimension") {
          ...
          case "unsigned int":
            ...
            // (dimensions and glues cast into integers) and plus & minus signs aren't allowed. This
            // context is used directly in the integer context and the factor context.
            mouth = new Mouth(this.string, this.queue);
            subContext = "start";
            digits = 0;
            found = false;
            ...
            while (true) {
              ...
              // If the current context is "grave", then the last token that was parsed was a grave
              // character (`). It makes the next token act as a number. If it's a character token,
              // the code point of the character is used. If it's a command token, the token can
              // only be one character in length and the code point of that one character is used
              // instead. This context has to come first because commands aren't supposed to be ex-
              // panded. If the command expansion if block came first, it would be incorrectly ex-
              // panded.
              if (subContext == "grave") {
                // Only a one-character-long command or an actual character can follow a grave char-
                // acter (`). If the command is more than one character long, then the whole number
                // search is aborted and null is returned.
                ...
                // gle character command. All that is handled above. This just changes the context.
                subContext = "grave";
              }
          ...
          case "factor":
            // This context is like a decimal context. It looks for any unsigned number and uses the
            // unsigned int context to look for regular integers. It can also find fractional deci-
            // mals though that use either a period or a comma as the decimal point. This context is
            // used in the dimension and glue context. The reason it's called "factor" instead of
            // "unsigned decimal" or something is because it is combined with a unit (e.g. 1.5em).
            // It acts as a factor for a unit. Since decimal values are allowed here, but integer
            // registers don't allow for decimals, the value gotten here is multiplied by 65536.
            // That way, a decimal like 0.5 can still be represented using the value 65536 * 0.5 =
            // 32768.
            mouth = new Mouth(this.string, this.queue);
            subContext = "start";
            digits = "";
      ...
      // The `preview` function basically calls the eat function, saves the token, reverts the eat-
      // ing, and returns the token. It lets the caller get the first token of the string/queue
      ...
      // will be rolled back to before the last token was parsed. This function does not revert fi-
      // nalized changes unless it's rolled back far enough and then finalized again. If a numerical
      // argument is provided, that function is repeated that many times.
      ...
      // this function will be finalized. If an input string was provided as an argument, it's up to
      // the calling function to change the original string since strings are immutable and the ref-
      // erence to the string can't be changed. If no string was provided though, the outer `string'
      ...
      // ment is not a command or active character token, the token is returned by itself in an ar-
      // ray. Otherwise, the command/active character token is looked up in the last Scope to re-
      // place it with its definition. The `mouth` argument is used in case the command is a primit-
      // ive that needs access to the next tokens. Usually though, the mouth isn't even used at all.
      function expand(token, mouth) {
        ...
        if (token && (token.type == "command" || token.type == "character" && token.cat ==
          ...
          if (token.invalid) {
            let tokens = (token.type == "command" ? token.escapeChar + token.name : token.char).split("");
          ...
          // A token can also be ignorable. In which case, just return an empty array. This is used
          // in case the token isn't invalid, but still needs to be skipped over.
          if (token.ignore) {
            return [{
            ...
            if (token.name in lastScope.defs.macros) {
              ...
              if (macro.type == "primitive") {
                ...
                for (let i = 0, l = macro.parameters.length; i < l; i++) {
                  ...
                  // other characters. If it is a parameter, the tokens in the macro call will act
                  // as arguments to be be used in the expansion text. If it's not a parameter char-
                  // acter, it should match exactly with the tokens after the macro call.
                  ...
                  if (tok.cat == catcodes.PARAMETER) {
                    ...
                    // single token or group is absorbed and used as the argument.
                    ...
                    if (macro.parameters[i + 1] &&
                      ...
                      let nextIndex = macro.parameters.map(token => token.cat == catcodes.PARAMETER
                      ).indexOf(true, i + 1);
                      ...
                      // This number keeps track of how many actual arguments were parsed. This may
                      // be different from the number of tokens that were parsed. For example, if
                      // "{hi}" is found, then it only counts as one argument, but as four tokens.
                      let tokensParsed = 0;
                      ...
                      while (true) {
                        ...
                        if (otherTok.length == 1) {
                          // To check if the next few tokens matches the `next` array of tokens, it-
                          // erate over the next few tokens in the stream and check if they match.
                          ...
                          // If the tokens all matched, go back once to before we had started scan-
                          // ning.
                          if (isDone) {
                            mouth.revert();
                            break;
                        ...
                        // The current token is not the delimiting closing token. It will be counted
                        // as part of the argument and the scanning should continue.
                        params[params.length - 1] = params[params.length - 1].concat(otherTok);

                        // Increment `tokensParsed`.
                        tokensParsed++;
                      }
                      // TeX removes the enclosing opening and closing tokens around an argument as
                      // long as it won't unbalance the group delimiters. If `tokensParsed' is just
                      // one, then opening and closing delimiters can be stripped. If it's more than
                      // one though, then they can't (because then there would be an unmatched clo-
                      // sing token in the middle and an unmatched opening token after it).
                      if (tokensParsed == 1 && params[params.length - 1][0].cat == catcodes.OPEN &&
                    ...
                    if (!tok ||
                      ...
                      // The token does not match. The macro call does not match its definition and
                      // an error would be thrown. Add an `invalid' property and return the initial
                      // token.
                      token.invalid = true;
                ...
                // is evaluate the replacement tokens. Parameter tokens followed by a number (e.g.
                // #1) will be replaced by the corresponding arguments. Parameter tokens followed by
                // another parameter token (e.g. ##) will be evaluated simply as a single parameter
                // token. That lets other \def commands happen in replacement tokens (e.g.
                ...
                for (let i = 0, l = macro.replacement.length; i < l; i++) {
                  ...
                  if (macro.replacement[i].cat == catcodes.PARAMETER) {
                    ...
                    // The token is a regular token. Clone it and add it directly to `replacement`.
                    // A clone is made in case there is a problem while parsing the replacement, to
                    // leave the original intact. If there *is* a problem, an `invalid` property
                    // might be added to a token, but that should not affect the original macro be-
                    // cause then all subsequent calls to the macro will also inherit that `invalid`
                    // property, even if it's not actually invalid.
                    let clone = {};
  ...
  function genHTML(container, tokens, contStyle, cssDeclaration) {
    ...
    // is created when a Rel or Bin atom is encountered so that a line only ends after characters
    // like "=" or "+".
    function newBox(tokens, style, cramped, font, parent) {
      ...
      for (let i = 0; i < tokens.length; i++) {
        ...
        if (token.type == "fraction") {
          ...
          // ured out by `fontTeX.fontDimen', but some just need to be known explicitly). Instead of
          ...
          let axisHeight = fontTeX.fontDimen.heightOf("x", family) / 2 +
              fontTeX.fontDimen.baselineHeightOf(family);
          ...
          let barWidthDimen =
              token.barWidth == "from font" ?
                new DimenReg(0, fontTeX.fontDimen.visibleWidthOf('|', family) * 65536) :
            ...
            switch (delimiter) {
              ...
              case '⟩':
                height =
                    Math.max(
                      setHeight,
                      fontTeX.fontDimen.heightOf(delimiter, family) +
                        fontTeX.fontDimen.depthOf(delimiter, family)
                    );
                ...
                elem.height = scale;
                elem.width =
                    fontTeX.fontDimen.widthOf(delimiter, family) /
                    (fontTeX.fontDimen.heightOf(delimiter, family) +
                      fontTeX.fontDimen.depthOf(delimiter, family)) * scale;
                ...
                context.font =
                    `${
                      scale / (fontTeX.fontDimen.heightOf(delimiter, family) +
                        fontTeX.fontDimen.depthOf(delimiter, family))
                    }px ${family}`;
                context.fillText(
                  delimiter,
                  elem.width / 2,
                  scale - fontTeX.fontDimen.depthOf(delimiter, family) * scale /
                    (fontTeX.fontDimen.heightOf(delimiter, family) +
                    fontTeX.fontDimen.depthOf(delimiter, family))
                );
              ...
              case ')':
                ...
                if (Math.floor(Math.floor(scale / 2 * height) - scale) <= 0) {
                  ...
                  elem.width =
                      fontTeX.fontDimen.widthOf(delimiter, family) /
                      fontTeX.fontDimen.heightOf(delimiter, family) * scale *
                      (fontTeX.fontDimen.heightOf(delimiter, family) +
                        fontTeX.fontDimen.depthOf(delimiter, family));
                  context = elem.getContext("2d");
                  context.textAlign = "center";
                  ...
                  context.fillText(
                    delimiter,
                    elem.width / 2,
                    scale / 2 - fontTeX.fontDimen.depthOf(delimiter, family) * scale * glyphHeight
                  );
                  ...
                  context.fillText(
                    delimiter,
                    elem.width / 2,
                    scale - fontTeX.fontDimen.depthOf(delimiter, family) * scale / glyphHeight
                  );
              ...
              case '⎰':
              case '⎱':
                axisHeight = fontTeX.fontDimen.heightOf("x", family) / 2;
                height =
                    Math.max(
                      setHeight,
                      fontTeX.fontDimen.heightOf(delimiter, family) +
              ...
              case '}':
                ...
                if (scale >= 3) {
                  context.fillText(
                    delimiter,
                    elem.width / 2,
                    Math.floor(scale / 3) - fontTeX.fontDimen.depthOf(delimiter, family) *
                      glyphHeight
                  );
                  ...
                  context.fillText(
                    delimiter,
                    elem.width / 2,
                    scale - fontTeX.fontDimen.depthOf(delimiter, family) * glyphHeight
                  );
                  ...
                  context.fillText(
                    delimiter,
                    elem.width / 2,
                    elem.height / 2 + scale / 2 - fontTeX.fontDimen.depthOf(delimiter, family) *
                     glyphHeight
                  );
                  context.clearRect(
                    0,
          ...
          let exHeight = fontTeX.fontDimen.heightOf("x", family) / 2;
        ...
        if (token.type == "table") {
          ...
          token.div.renderedHeight = token.div.offsetHeight / fontSize / 2 +
              fontTeX.fontDimen.heightOf("x", family) / 2;
        ...
        if (token.type == "atom" || token.type == "box" && token.content.type == "atom") {
            ...
            if (token.nucleus.code == 0x000A /* New Line character U+000A */) {
                  ...
                  `<div style="white-space:pre;display:inline-block;${
                    ...
                    nm: token.atomType == atomTypes.VARIABLE ?
                      `font-style:italic"><div style="display:inline-block;margin:0 ${
                        ...
                        fontTeX.fontDimen.italCorrOf(token.nucleus.char, family)
                      }em 0 ${
                        // This aligns a character to the left side of its boundary box.
                        fontTeX.fontDimen.leftOffsetOf(token.nucleus.char, family, 'it')
                      }em` : "",
                    rm: "",
              ...
              let fontStyle =
                  font == "nm" ?
                    ...
                    font;
              token.div.renderedHeight =
                  fontTeX.fontDimen.heightOf(token.nucleus.char, family, fontStyle) * multiplier;
              token.div.renderedDepth =
                  fontTeX.fontDimen.trueDepthOf(token.nucleus.char, family, fontStyle) * multiplier;
              token.div.baseline =
                  fontTeX.fontDimen.baselineHeightOf(family) * multiplier;

              scriptOffset = fontTeX.fontDimen.scriptOffsetOf(
                  token.nucleus.char,
                  family,
          ...
          if (token.delimited) {
            ...
            function renderElem(elem, delimiter, leftSide, scale) {
              ...
              switch (delimiter) {
                ...
                case "⟨":
                case "⟩":
                  // This is the simplest case (other than the one with no delimiter). If the char-
                  // acter falls into this category, it's just stretched. Nothing special. The whole
                  ...
                  // that a character like a "-" won't just have a super small, weird-looking delim-
                  // iter.
                  axisHeight = fontTeX.fontDimen.heightOf("x", family) / 2;
                  height =
                      Math.max(
                        ...
                        (fontTeX.fontDimen.heightOf(delimiter, family) +
                          fontTeX.fontDimen.depthOf(delimiter, family)) / 2
                      ) * 2;

                  elem.style.height = `${height}em`;

                  elem.style.width = `${fontTeX.fontDimen.visibleWidthOf(delimiter, family)}em`;
                  elem.style.marginLeft = `${-fontTeX.fontDimen.leftOffsetOf(delimiter, family)}em`;
                  elem.style.marginRight =
                      `${fontTeX.fontDimen.widthOf(delimiter, family) -
                        fontTeX.fontDimen.visibleWidthOf(delimiter, family) +
                        fontTeX.fontDimen.leftOffsetOf(delimiter, family)
                      }em`;

                  //elem.style.width = `${fontTeX.fontDimen.widthOf(delimiter, family)}em`;
                  elem.style.verticalAlign = "middle";

                  // If an atom has a height or depth lower than a delimiter, the delimiter charac-
                  // ter adds extra height/depth.
                  token.div.renderedHeight =
                  ...
                  elem.height = scale;
                  // Gets the width in terms of the height.
                  elem.width =
                      fontTeX.fontDimen.visibleWidthOf(delimiter, family) /
                      (fontTeX.fontDimen.heightOf(delimiter, family) +
                      fontTeX.fontDimen.depthOf(delimiter, family)) * scale;
                  ...
                  context.fillText(
                    delimiter,
                    fontTeX.fontDimen.leftOffsetOf(delimiter, family) * scale,
                    scale - fontTeX.fontDimen.depthOf(delimiter, family) * scale /
                      (fontTeX.fontDimen.heightOf(delimiter, family) +
                      fontTeX.fontDimen.depthOf(delimiter, family))
                  );
                ...
                case ')':
                  ...
                  if (Math.floor(Math.floor(scale / 2 * height) - scale) <= 0) {
                    ...
                    // the rectangle between their two halves, only one row of one pixels will actu-
                    // ally be cleared. That's a problem because the top character will also be vis-
                    // ible in the bottom character's space because they're too close together. To
                    // prevent that, the bottom is drawn first (only the bottom half), copied in an
                    ...
                    // below because it liter ally sets every single RGBA channel of every single
                ...
                case '⎱':
                  ...
                  // The code below is coped from the parenthesis case.
                ...
                case '}':
                  ...
                  // other than analyzing individual pixels, but it works for almost every case so
                  // meh.
                  ...
                  if (scale >= 3) {
                    ...
                    // Since there's three parts, the top needs to be saved and erased too for the
                    // next part (the middle) to be drawn.
                    context.fillText(
                      delimiter,
                      elem.width / 2,
                      scale - fontTeX.fontDimen.depthOf(delimiter, family) * glyphHeight
                    ...
                    // Place the two saved sections now.
                    context.putImageData(topHalf, 0, 0);
                    context.putImageData(bottomHalf, 0, elem.height - Math.floor(scale / 3));

                    // This checks if the region in between is 0 pixels tall and skips over connect-
                    // ing the regions if it is (since nothing would be drawn).
                    if (elem.height > scale) {
                      ...
                      for (let i = 0, l = topRegion.height / 2; i < l; i++) {
                        for (let n = 0, j = elem.width; n < j; n++) {
                          let pixelOffset = i * elem.width * 4 + n * 4;
                          topRegion.data[pixelOffset + 0 /* red   */] = topHalfRow[n * 4 + 0];
                          topRegion.data[pixelOffset + 1 /* green */] = topHalfRow[n * 4 + 1];
                          topRegion.data[pixelOffset + 2 /* blue  */] = topHalfRow[n * 4 + 2];
                          topRegion.data[pixelOffset + 3 /* alpha */] = topHalfRow[n * 4 + 3];
                      ...
                      // Now do the same for the bottom fill section.
                      bottomRegion =
                          context.createImageData(
                            elem.width,
                            Math.floor(elem.height / 2) - Math.floor(scale / 2) + 1
            ...
            if (token.atomType == atomTypes.OP && token.limits) {
              ...
              if (token.subscript && !token.superscript) {
                ...
                if (style == "scriptscript") {
                  ...
                  sub.style.top =
                      `${Math.max(sub.baseline, sub.renderedDepth) -
                        ...
                        Math.max(
                          ...
                          sub.renderedHeight - fontTeX.fontDimen.heightOf("x", family)
                        ) - height
                      }em`;
                  heightOffset.style.paddingBottom =
                      `${Math.max(sub.baseline, sub.renderedDepth) -
                  ...
                  sub.style.top =
                      `${Math.max(sub.baseline, sub.renderedDepth) -
                        fontTeX.fontDimen.baselineHeightOf(family) / rootHalf +
                        Math.max(
                      ...
                      `${Math.max(sup.baseline, sup.renderedDepth) -
                        ...
                        Math.max(
                          ...
                          sup.renderedDepth + (cramped ? .9 : 1) *
                            fontTeX.fontDimen.heightOf("x", family)
                        )
                      }em`;
                ...
                let exHeight = fontTeX.fontDimen.heightOf("x", family);
                if (style == "scriptscript") {
                  sub.style.fontSize =
                      sup.style.fontSize = "";
                  ...
                  sub.style.top =
                      `${Math.max(sub.baseline, sub.renderedDepth) -
                        fontTeX.fontDimen.baselineHeightOf(family) / rootHalf + Math.max(
                          Math.max(token.div.renderedDepth, 0) / multiplier / rootHalf + .2,
                          sub.renderedHeight - .8 * exHeight
                        ) - depth
                      }em`;
                  heightOffset.style.paddingBottom =
                      `${Math.max(sub.baseline, sub.renderedDepth) * rootHalf -
                        fontTeX.fontDimen.baselineHeightOf(family) + Math.max(
                          Math.max(token.div.renderedDepth, 0) / multiplier / rootHalf + .2,
                          sub.renderedHeight - .8 * exHeight
                        ) * rootHalf
                      }em`;
          ...
          if (box) {
            if (box.boxType == "horizontal") {
              ...
              if (height.em.value < 0) {
                ...
                token.div.firstElementChild.style.verticalAlign = "text-bottom";
                let baselineHeight = fontTeX.fontDimen.baselineHeightOf(family);
                token.div.renderedDepth =
          ...
          switch (token.atomType) {
            case atomTypes.OVER:
              ...
              lineWidth = fontTeX.fontDimen.visibleWidthOf("|", family);
              ...
              fullContainer.style.top =
                  `${-fontTeX.fontDimen.baselineHeightOf(family) - token.div.renderedHeight /
            ...
            case atomTypes.UNDER:
              ...
              fullContainer.style.position = "relative";
              fullContainer.style.top =
                  `${-fontTeX.fontDimen.baselineHeightOf(family) + token.div.renderedDepth /
                    multiplier + .12}em`;
              fullContainer.style.verticalAlign = "text-bottom";
              fullContainer.style.pointerEvents = "none";
              ...
              heightOffset.innerText = "\u00A0";
              heightOffset.style.paddingBottom =
                  `${lineWidth + .16 - fontTeX.fontDimen.baselineHeightOf(family) +
            ...
            case atomTypes.ACC:
              ...
              acc.style.top =
                `${Math.min(fontTeX.fontDimen.heightOf("x", family), token.div.renderedHeight) -
                  token.div.renderedHeight}em`;
              acc.style.width = 0;
              ...
              offset = token.nucleus &&
                  (token.nucleus.type == "symbol" && (font == "it" || font == "sl")) ?
                    fontTeX.fontDimen.italCorrOf(token.accChar, family) :
                    0;
              offset = offset ||
              ...
              acc.style.left =
                  `${
                    ((token.div.offsetWidth + 1) / 50 -
                      fontTeX.fontDimen.widthOf(token.accChar, family, font)) / 2 + offset
                  }em`;
              container.removeChild(token.div);
              token.div.style.fontSize = oldFontSize;
              ...
              spacer.style.height =
                  `${(token.div.renderedHeight - Math.min(
                    fontTeX.fontDimen.heightOf("x", family),
                    token.div.renderedHeight
                    )) + fontTeX.fontDimen.heightOf(token.accChar, family, font)
                }em`;
            ...
            case atomTypes.RAD:
              ...
              lineWidth = fontTeX.fontDimen.visibleWidthOf("|", family);
              token.div.renderedHeight =
                  Math.max(token.div.renderedHeight || 0, fontTeX.fontDimen.heightOf("x", family) / 2);
              token.div.renderedDepth = Math.max(token.div.renderedDepth || 0, 0);
              ...
              if (settings["radical.build"][0]) {
                ...
                canvas.height = (fontTeX.fontDimen.heightOf("√", family) +
                    fontTeX.fontDimen.depthOf("√", family)) * fontSize;
                ...
                context.fillText(
                  "√",
                  canvas.width / 2,
                  canvas.height * (1 - fontTeX.fontDimen.depthOf("√", family))
                );
              }
      ...
      for (let i = 0, l = items.length; i < l; i++) {
        ...
        if (token.italicCorrection) {
          token.dimen = new DimenReg(
            0,
            fontTeX.fontDimen.italCorrOf(token.italicCorrection, family) * 65536
          );
        }
    ...
    defs: {
      primitive: {
        ...
        "/": new Primitive("/", function(e) {
          ...
          // correction has to be determined manually (by `fontTeX.fontDimen.italCorrOf').
  ...
  // `fontTeX.fontDimen` is used to measure font characters. TeX fonts usually have certain parame-
  ...
  fontTeX.fontDimen = {
    ...
    italCorrOf: function(string, family) {
      ...
      return this.cache[family].italic[string].italCorr = Math.max(
        0,
        measure.call(this, .5, .5, 0) * 1.5 - fontTeX.fontDimen.widthOf(string, family, "it")
      );
    },
  ...
  bodyLoaded.then(function(body) {
    ...
    canvas.width = 150;
    canvas.height = 150;
    fontTeX.fontDimen.canvas = canvas;
    fontTeX.fontDimen.context = canvas.getContext("2d");
  ...
  function onFontLoad(fontStack, callback) {
    ...
    let fonts = fontStack.split(", ").map(font =>
      font[0] == '"' ? font.substring(1, font.length - 1) : font
    );
  ...
  // Deletes the cache in fontTeX.fontDimen (useful when a new font has loaded and new measurements
  // need to be gotten).
  function clearFontCache(family, style) {
    ...
    // Delete only the cache associated with a certain style.
      ...
      if (style.length) {
        for (let family in fontTeX.fontDimen.cache) {
